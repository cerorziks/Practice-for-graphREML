...
Cite GraphLD/graphREML, the GWAS source, annotation resource, and LD reference resource. Follow the original GWAS data-use restrictions. Do not commit restricted summary statistics, participant-level data, credentials, or large reference downloads to GitHub.

Add a repository `LICENSE`, `CITATION.cff`, and `.gitignore` appropriate for your project.
